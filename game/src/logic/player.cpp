...
namespace engine {
Player::Player()
    : angle(0.0f), textureIndex(0), respawnTicks(0), isFlying(false),
      isFlipped(false), onGround(false), onCeiling(false), isNpc(false),
...
void Player::update() {
  ...
  if (!isFlying) {
    ...
    // Flattens the angle out once on the ground.
      ...
      gs::util::approach(&angle, std::round(angle / 90.0f) * 90.0f, 50.0f);

    // Leaves a trail of particles while the player is on the ground.
    if (!isClone && onGround && ticks % 2 == 0)
      Particle::spawnParticles(position + gs::Vec2f(-0.3f, 0.3f),
    ...
    if (jump) {
      if (!isClone && ticks % 2 == 0)
        Particle::spawnParticles(
            position +
                gs::Vec2f(std::cos(gs::util::toRadians(angle + 140.0f)) * 0.5f,
                          std::sin(gs::util::toRadians(angle + 140.0f)) *
                              0.5f) +
  ...
  if (!isClone && !prvsOnGround && onGround)
    Particle::spawnParticles(position + gs::Vec2f(0.0f, 0.3f),
                             ParticleInfo::GroundHit, secondaryColor, 15);
...
void Player::updatePosition() { position += velocity; }
