...
#include "../../hdr/resources.hpp"

namespace engine {
Tile::CollisionType Tile::getCollisionType(Id id) {
  return tiles[id].collisionType;
}
Tile::ActionType Tile::getActionType(Id id) { return tiles[id].actionType; }
int Tile::getTextureIndex(Id id) { return tiles[id].textureIndex; }
bool Tile::isColorable(Id id) { return tiles[id].colorable; }
...
Map::Map()
    : size(gs::Vec2i(50, 25)), mapNumber(0), hue(210.0f), tiles(nullptr),
      blockSize(0) {}
Map::~Map() {
  // Cleans up memory.
  deallocate();
  ...
  if (tilesCopy != nullptr) {
    for (int xpos = 0; xpos < sizeCopy.x; xpos++) {
      for (int ypos = 0; ypos < sizeCopy.y; ypos++) {
        ...
        setTile({xpos + xposOffset, ypos},
                tilesCopy[(ypos * sizeCopy.x) + xpos + xArrayOffset]);
      }
    }
...
void Map::deallocate() { delete[] tiles; }
